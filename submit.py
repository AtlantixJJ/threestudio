...
def zero123_comp():
    image_paths = [
        'train_face_rgba.png',
        'face_centered_rgba.png'
    ]
    models = [
        '../zero123/zero123/logs/2024-02-08T20-06-11_sd-face/checkpoints/last.ckpt',
    ...
    cmd = "python launch.py --config configs/stable-zero123.yaml --train --gpu 0 data.image_path=../dreamgaussian/data/{image_path} system.guidance.pretrained_model_name_or_path={model} exp_root_dir=outputs/{name}"
    for image_path in image_paths:
        for model, name in zip(models, model_name):
            yield cmd.format(image_path=image_path, model=model, name=name)
